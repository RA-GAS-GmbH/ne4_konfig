...
pub async fn read_value(context: &mut client::Context) -> tokio::io::Result<Vec<u16>> {
    context
        .read_input_registers(VALUE_RREG_START, VALUE_RREG_COUNT)
        .await
}

pub async fn read_value_with_timeout(
    context: &mut client::Context,
    timeo: Duration,
) -> tokio::io::Result<Vec<u16>> {
    timeout(timeo, read_value(context)).await?
}
